...
async def evaluate_caregiver_match(
    ...
    try:
        ...
        # 1. 서비스 유형 필터링
        if not _filter_by_service_type(structured_preferences, request):
            logger.debug(f"요양보호사 {caregiver.caregiver_id} 서비스 유형 불일치")
            return False
        
        # 2. 시간대 필터링
        if not _filter_by_time_preferences(structured_preferences, request):
            logger.debug(f"요양보호사 {caregiver.caregiver_id} 시간대 불일치")
            return False
        
        # 3. 지원 질환 필터링
        if not _filter_by_supported_conditions(structured_preferences, request):
            logger.debug(f"요양보호사 {caregiver.caregiver_id} 지원 질환 불일치")
            return False
        
        # 4. 선호 연령/성별 필터링
        if not _filter_by_preferred_demographics(structured_preferences, request):
            logger.debug(f"요양보호사 {caregiver.caregiver_id} 선호 연령/성별 불일치")
            return False
        
        # 5. 요일 필터링
        if not _filter_by_day_of_week(structured_preferences, request):
            logger.debug(f"요양보호사 {caregiver.caregiver_id} 요일 불일치")
            return False
        
        logger.debug(f"요양보호사 {caregiver.caregiver_id} 모든 필터 조건 통과")
        return True
...
def _filter_by_service_type(structured_preferences: Any, request: MatchingRequestDTO) -> bool:
    """서비스 유형 필터링: 요양보호사의 선호 서비스 유형과 요청 서비스 유형 매칭"""
    # 선호조건에 service_types가 없거나 비어있으면 기본 통과
    if not hasattr(structured_preferences, 'service_types') or not structured_preferences.service_types:
        return True
    
    # 요청에 serviceType이 없으면 기본 통과
    if not request.serviceRequest.serviceType:
        return True
    
    # 요청 서비스 유형이 요양보호사의 선호 서비스 유형 목록에 있는지 확인
    return request.serviceRequest.serviceType in structured_preferences.service_types

def _filter_by_time_preferences(structured_preferences: Any, request: MatchingRequestDTO) -> bool:
    """시간대 필터링: 요양보호사의 근무 시간대와 요청 시간대 매칭"""
    # 선호조건에 work_start_time 또는 work_end_time이 없으면 기본 통과
    if not hasattr(structured_preferences, 'work_start_time') or not structured_preferences.work_start_time:
        return True
    if not hasattr(structured_preferences, 'work_end_time') or not structured_preferences.work_end_time:
        return True
    
    # 요청에 preferredStartTime 또는 preferredEndTime이 없으면 기본 통과
    if not request.serviceRequest.preferredStartTime or not request.serviceRequest.preferredEndTime:
        return True
    
    # 시간대 비교 로직 (간단한 문자열 비교로 구현, 실제로는 시간 파싱 및 비교 필요)
    # 여기서는 단순히 시간 문자열이 겹치는지 확인하는 예시
    pref_start = structured_preferences.work_start_time
    pref_end = structured_preferences.work_end_time
    req_start = request.serviceRequest.preferredStartTime
    req_end = request.serviceRequest.preferredEndTime
    
    # 시간대가 겹치는지 확인 (예: 요청 시작시간이 선호 종료시간 전이고, 요청 종료시간이 선호 시작시간 후)
    return req_start <= pref_end and req_end >= pref_start

def _filter_by_supported_conditions(structured_preferences: Any, request: MatchingRequestDTO) -> bool:
    """지원 질환 필터링: 요양보호사의 지원 질환과 요청 추가 정보 매칭"""
    # 선호조건에 supported_conditions가 없거나 비어있으면 기본 통과
    if not hasattr(structured_preferences, 'supported_conditions') or not structured_preferences.supported_conditions:
        return True
    
    # 요청에 additionalInformation이 없으면 기본 통과
    if not request.serviceRequest.additionalInformation:
        return True
    
    # 간단한 키워드 매칭으로 구현 (실제로는 더 정교한 NLP 처리 필요)
    additional_info = request.serviceRequest.additionalInformation.lower()
    for condition in structured_preferences.supported_conditions:
        if condition.lower() in additional_info:
            return True
    
    return False

def _filter_by_preferred_demographics(structured_preferences: Any, request: MatchingRequestDTO) -> bool:
...
def _filter_by_day_of_week(structured_preferences: Any, request: MatchingRequestDTO) -> bool:
