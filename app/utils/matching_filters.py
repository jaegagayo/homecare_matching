...
async def evaluate_caregiver_match(
    ...
    try:
        ...
        # 1. 서비스 유형 필터링
        if not filter_by_service_type(structured_preferences, request):
            logger.debug(f"요양보호사 {caregiver.caregiver_id} 서비스 유형 불일치")
            return False
        
        # 2. 시간대 필터링
        if not filter_by_time_preferences(structured_preferences, request):
            logger.debug(f"요양보호사 {caregiver.caregiver_id} 시간대 불일치")
            return False
        
        # 3. 지원 질환 필터링
        if not filter_by_supported_conditions(structured_preferences, request):
...
def filter_by_service_type(structured_preferences: Any, request: MatchingRequestDTO) -> bool:
...
def filter_by_time_preferences(structured_preferences: Any, request: MatchingRequestDTO) -> bool:
...
def filter_by_supported_conditions(structured_preferences: Any, request: MatchingRequestDTO) -> bool:
