...
class ServiceRequestDTO(BaseModel):
    ...
    location: LocationInfo = Field(..., description="서비스 요청 위치 정보")
...
class CaregiverForMatchingDTO(BaseModel):
    ...
    baseLocation: LocationInfo = Field(..., description="활동 지역 위치 정보")
...
class MatchedCaregiverDTO(BaseModel):
    ...
    availableTimes: Optional[str] = Field(None, description="가능 시간")
    address: Optional[str] = Field(None, description="주소")
    location: Optional[LocationInfo] = Field(None, description="위치 정보")
